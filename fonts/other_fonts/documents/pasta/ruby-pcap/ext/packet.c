 ...
 */
VALUE
new_packet( const u_char *data, const struct pcap_pkthdr *pkthdr, int dl_type)
{
    VALUE class;
    struct packet_object *pkt;
    ...
    if (dl_type > DATALINK_MAX && dl_type != LINKTYPE_LINUX_SLL) {
        rb_raise(ePcapError, "Unknown data-link type (%d)", dl_type);
    }
    if(dl_type == LINKTYPE_LINUX_SLL){
      nltype_off = 14;
      nl_off = 16;
    }else{
    ...
    for(;;){
        switch (check_tpid_pattern(nl_type)){
        case VLAN:
            nltype_off += VLAN_FRAME_SIZE;
            nl_off += VLAN_FRAME_SIZE;
            nl_type = ntohs(*(u_short *)(data + nltype_off));
            continue;
        case EOE:
            nltype_off += EOE_FRAME_SIZE;
            nl_off += EOE_FRAME_SIZE;
            nl_type = ntohs(*(u_short *)(data + nltype_off));
            continue;
        case PBB:
            nltype_off += PBB_FRAME_SIZE;
            nl_off += PBB_FRAME_SIZE;
            nl_type = ntohs(*(u_short *)(data + nltype_off));
            continue;
        case DEFAULT:
            break;
...
static VALUE packet_datalink(VALUE self);
/* Return true if this packet is an instance of {IPPacket} or its sub-class. */
static VALUE packet_ip(VALUE self);
/* Return true if this packet is an instance of {TCPPacket} or its sub-class. */
static VALUE packet_tcp(VALUE self);
/* Return true if this packet is an instance of {UDPPacket} or its sub-class. */
static VALUE packet_udp(VALUE self);
/* Return total length of packet. */
static VALUE packet_length(VALUE self);
/* Return captured length. */
static VALUE packet_caplen(VALUE self);
/* Return timestamp as {http://www.ruby-doc.org/core-1.9.3/Time.html Time}. */
static VALUE packet_time(VALUE self);
/* Return timestamp as integer. */
static VALUE packet_time_i(VALUE self);
/* Return raw data of packet as {http://www.ruby-doc.org/core-1.9.3/String.html String}. */
static VALUE packet_raw_data(VALUE self);
...
/* Return as {http://www.ruby-doc.org/core-1.9.3/Array.html Array} of objects of {EthernetHeader}. */
static VALUE
ethernet_headers(VALUE self)
{
    struct packet_object *pkt;

    DEBUG_PRINT("ethernet_headers");

    GetPacket(self, pkt);
    if (pkt->hdr.dl_type != LINKTYPE_ETHERNET){
        return Qnil;
    }
    return setup_ethernet_headers((struct ether_header *)LAYER2_HDR(pkt));
}

/* Return ethernet data of packet as {http://www.ruby-doc.org/core-1.9.3/String.html String}. */
static VALUE
ethernet_data(VALUE self)
{
    struct packet_object *pkt;
    int offset;

    DEBUG_PRINT("ethernet_data");

    GetPacket(self, pkt);
    if (pkt->hdr.dl_type != LINKTYPE_ETHERNET){
        return Qnil;
    }
    offset = get_ethernet_header_length((struct ether_header *)LAYER2_HDR(pkt));
    return rb_str_new(((u_char *)LAYER2_HDR(pkt) + offset), (pkt->hdr.pkthdr.caplen - offset));
}

/* Return size of padding area of ehternet packet as integer. */
static VALUE
ethernet_padding_size(VALUE self)
{
    struct packet_object *pkt;
    int caplen;
    int ethernet_hlen;
    ...
    DEBUG_PRINT("ethernet_padding_size");

    GetPacket(self, pkt);
    if (pkt->hdr.dl_type != LINKTYPE_ETHERNET){
        return Qnil;
    }
    caplen = pkt->hdr.pkthdr.caplen;
    ethernet_hlen = get_ethernet_header_length((struct ether_header *)LAYER2_HDR(pkt));

    /* Check ARP */
#ifdef ETHERTYPE_ARP
    if (ntohs(*(u_short *)(LAYER2_HDR(pkt) + ethernet_hlen - TPID_SIZE)) == ETHERTYPE_ARP){
        data_len = ARP_HEADER_SIZE;
    } else{
        data_len = get_total_length(pkt);
    }
#else
    if (ntohs(*(u_short *)(LAYER2_HDR(pkt) + ethernet_hlen - TPID_SIZE)) == ETH_P_ARP){
        data_len = ARP_HEADER_SIZE;
    } else{
        data_len = get_total_length(pkt);
    }
    ...
    total_len = ethernet_hlen + data_len;
    if (caplen == total_len){
        return INT2FIX(0);
    } else if (total_len <= ETHER_MIN_LEN){
        return INT2FIX((ETHER_MIN_LEN - total_len));
    } else{
        return INT2FIX(0);
    }
}

/* Return true if linktype of this packet is ethernet. */
static VALUE
ethernet_check(VALUE self)
{
    struct packet_object *pkt;

    DEBUG_PRINT("ethernet_check");
...
static VALUE
ethernet_dst_mac_address(VALUE self)
{
    struct packet_object *pkt;
    char mac_addr[32];

    DEBUG_PRINT("ethernet_dst_mac_address");
...
static VALUE
ethernet_src_mac_address(VALUE self)
{
    struct packet_object *pkt;
    char mac_addr[32];

    DEBUG_PRINT("ethernet_src_mac_address");
...
Init_packet(void)
{
    ...
    /* define class Packet */
    /* Packet captured with {Capture} object. Packet and its sub-classes are {http://ruby-doc.org/core-1.9.3/Marshal.html Marshal} compliant. */
    ...
    /* include Ethernet Module */
    mEthernet = rb_define_module_under(cPacket, "Ethernet");
    rb_include_module(cPacket, mEthernet);

    rb_define_method(mEthernet, "ethernet_headers", ethernet_headers, 0);
    rb_define_method(mEthernet, "ethernet_data", ethernet_data, 0);
    rb_define_method(mEthernet, "padding_size", ethernet_padding_size, 0);
    rb_define_method(mEthernet, "ethernet?", ethernet_check, 0);
    rb_define_method(mEthernet, "dst_mac", ethernet_dst_mac_address, 0);
    rb_define_alias(mEthernet,  "dst_mac_address", "dst_mac");
    rb_define_method(mEthernet, "src_mac", ethernet_src_mac_address, 0);
    rb_define_alias(mEthernet,  "src_mac_address", "src_mac");
