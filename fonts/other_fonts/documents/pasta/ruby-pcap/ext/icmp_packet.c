...
/* Return ICMP type. */
static VALUE icmpp_type(VALUE self);
/* Return ICMP code. */
static VALUE icmpp_code(VALUE self);
/* Return ICMP checksum. */
static VALUE icmpp_cksum(VALUE self);
/* Return string describing ICMP type (e.g. "echo reply"). */
static VALUE icmpp_typestr(VALUE self);
...
static VALUE icmpp_pptr(VALUE self);
/* Return gateway address of ICMP_REDIRECT message as {IPAddr}. */
static VALUE icmpp_gwaddr(VALUE self);
/* Return identifier. */
static VALUE icmpp_id(VALUE self);
/* Return sequence number. */
static VALUE icmpp_seq(VALUE self);
/* 
 ...
 */
static VALUE icmpp_seqle(VALUE self);
...
static VALUE icmpp_num_addrs(VALUE self);
/* Return word per address entry of ICMP_ROUTERADVERT message. */
static VALUE icmpp_wpa(VALUE self);
/* Return lifetime of ICMP_ROUTERADVERT message. */
static VALUE icmpp_lifetime(VALUE self);
 ...
 *
 * @param [Integer] nth nth addres entry
 */
static VALUE
icmpp_radv(self, ind)
     VALUE self, ind;
...
/* Return timestamp of ICMP_TSTAMP/ICMP_TSTAMPREPLY message as {http://www.ruby-doc.org/core-1.9.3/Time.html Time}. */
static VALUE icmpp_otime(VALUE self);
/* Return timestamp of ICMP_TSTAMP/ICMP_TSTAMPREPLY message as {http://www.ruby-doc.org/core-1.9.3/Time.html Time}. */
static VALUE icmpp_rtime(VALUE self);
/* Return timestamp of ICMP_TSTAMP/ICMP_TSTAMPREPLY message as {http://www.ruby-doc.org/core-1.9.3/Time.html Time}. */
static VALUE icmpp_ttime(VALUE self);
...
/* Return data portion of ICMP_ECHO/ICMP_ECHOREPLY messages as {http://www.ruby-doc.org/core-1.9.3/String.html     String} */
static VALUE icmpp_data(VALUE self);
...
static VALUE icmpp_ttl(self)
     VALUE self;
{
    struct packet_object *pkt;
    struct icmp *icmp;
    GetPacket(self, pkt);
    CheckTruncateICMP(pkt, 12);
    icmp = ICMP_HDR(pkt);
    return (UINT32_2_NUM(ntohl(DOMAIN(icmp)->ttl)));
}
